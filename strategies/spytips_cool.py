...
def spy_tips_cool():
    ...
    if not os.path.exists(fileName):
        ...
        for i in range(2, min(len(spy_diff), len(tips_diff))):
            if (spy_diff.iloc[-i] > 0 and tips_diff.iloc[-i] > 0) == (spy_diff.iloc[-i + 1] > 0 and tips_diff.iloc[-i + 1] > 0):
                consecutive_days += 1
            else:
                consecutive_days = 1
    ...
    if last_entry is None:
        ...
        subject2 = ""
        text = f"{'Currently in market' if new_entry[5] else 'Currently in cash'} ({new_entry[6]} cooldown days remaining)\n"
        text += f"The SIGNAL is {total_indicator} {signal_emoji}\n"
        text += f"The SPY signal is {spy_indicator} {spy_emoji} with a difference of {spy_diff.iloc[-1]:.2%}\n"
        text += f"The TIPS signal is {tips_indicator} {tips_emoji} with a difference of {tips_diff.iloc[-1]:.2%}\n"
    ...
    text = f"{'Currently in market' if new_entry[5] else 'Currently in cash'} ({new_entry[6]} cooldown days remaining)\n"
    text += f"The SIGNAL {'remains' if total_indicator == last_indicator else 'has changed to'} {total_indicator} {signal_emoji}\n"
    text += f"The SPY signal {'remains' if spy_indicator == last_spy_indicator else 'has changed to'} {spy_indicator} {spy_emoji} with a difference of {spy_diff.iloc[-1]:.2%}\n"
    text += f"The TIPS signal {'remains' if tips_indicator == last_tips_indicator else 'has changed to'} {tips_indicator} {tips_emoji} with a difference of {tips_diff.iloc[-1]:.2%}\n"
