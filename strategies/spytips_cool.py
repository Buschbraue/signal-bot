...
def spy_tips_cool():
    ...
    if last_entry is None:
        ...
        text += f"The SIGNAL is {total_indicator} {signal_to_emoji(total_indicator)}\n"
        text += f"The SPY signal is {spy_indicator} {signal_to_emoji(spy_indicator)} with a difference of {spy_diff.iloc[-1]:.2%}\n"
        text += f"The TIPS signal is {tips_indicator} {signal_to_emoji(tips_indicator)} with a difference of {tips_diff.iloc[-1]:.2%}\n"
    ...
    text = f"Currently in market ({new_entry[6]} cooldown days remaining)\n" if new_entry[5] else f"Currently in cash ({new_entry[6]} cooldown days remaining)\n"
    text += (
        f"The SIGNAL remains {total_indicator} {signal_to_emoji(total_indicator)}\n"
        if total_indicator == last_indicator
        else f"The SIGNAL has changed to {total_indicator} {signal_to_emoji(total_indicator)}\n"
    )
    text += (
        f"The SPY signal remains {spy_indicator} {signal_to_emoji(spy_indicator)}"
        if spy_indicator == last_spy_indicator
        else f"The SPY signal has changed to {spy_indicator} {signal_to_emoji(spy_indicator)}"
    )
    text += f" with a difference of {spy_diff.iloc[-1]:.2%}\n"
    text += (
        f"The TIPS signal remains {tips_indicator} {signal_to_emoji(tips_indicator)}"
        if tips_indicator == last_tips_indicator
        else f"The TIPS signal has changed to {tips_indicator} {signal_to_emoji(tips_indicator)}"
    )
